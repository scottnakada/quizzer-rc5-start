  ...
  constructor(private backend:MockBackend) {
    ...
    this.backend.connections.subscribe(c => {
      ...
      if (c.request.url === "api/quizzes" && c.request.method === 0) {
        c.mockRespond(new ResponseOptions({
          body: JSON.stringify(quizList)
        }));
      }
      else if (c.request.url.match(singleQuizMatcher) && c.request.method === 0) {
        let matches = quizList.filter((t) => {
          return t._id == c.request.url.match(singleQuizMatcher)[1]
        });
